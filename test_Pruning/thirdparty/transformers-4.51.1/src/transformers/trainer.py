...
from transformers.dummy import GaussianStratifiedSampler,ZipfSampler
...
class CustomTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.teacher_model = BaseLlamaForCausalLM.from_pretrained("/data/kris/shared_data/models/Llama-3.2-3B",
        ...
        tokenizer = AutoTokenizer.from_pretrained("/data/kris/shared_data/models/Llama-3.2-3B")
...
# class CustomTrainer(Trainer):
#     """
#     一个自定义的Trainer，它在计算损失时加入了PCA正则化项。
#     """
...
#             pca_results_path (str): 保存PCA均值和主成分的 .npz 文件路径。
#             target_layers (List[int]): 需要施加PCA约束的解码器层索引列表。
#             pca_lambda (float): PCA正则化损失的权重系数。
#         """
#         super().__init__(*args, **kwargs)

#         pca_results_path = os.environ.get('PCA_RESULTS_PATH')

#         if target_layers is None:
#             self.target_layers = [5,8,11]
#         else:
#             self.target_layers = target_layers

#         self.pca_lambda = 0.0
        
#         print("--- 正在加载并准备PCA正则化参数 ---")
#         self.pca_params = self._load_and_prepare_pca_params(pca_results_path)
#         print(f"PCA参数加载完成，将在设备 {self.model.device} 上对层 {self.target_layers} 进行正则化。")

#     def _load_and_prepare_pca_params(self, npz_path: str) -> Dict[str, Dict[str, torch.Tensor]]:
#         """加载NumPy格式的PCA参数，并将其转换为GPU上的PyTorch张量。"""
#         if not os.path.exists(npz_path):
#             raise FileNotFoundError(f"PCA结果文件未找到: {npz_path}")
            
#         pca_data = np.load(npz_path, allow_pickle=True)
        
#         # .item() 用于从0维NumPy数组中提取字典
#         pca_components = pca_data['components'].item()
#         pca_mean = pca_data['mean'].item()
        
#         prepared_params = {}
#         for layer_idx in self.target_layers:
#             if layer_idx not in pca_components or layer_idx not in pca_mean:
#                 raise ValueError(f"目标层 {layer_idx} 的PCA参数未在文件中找到。")
            
#             # 关键：转换为PyTorch张量，并移动到与模型相同的设备和数据类型
#             prepared_params[layer_idx] = {
#                 "mean": torch.tensor(pca_mean[layer_idx], 
#                                      device=self.model.device, 
#                                      dtype=self.model.dtype),
#                 "components": torch.tensor(pca_components[layer_idx], 
#                                            device=self.model.device, 
#                                            dtype=self.model.dtype)
#             }
#         return prepared_params

#     def _compute_single_layer_pca_loss(
#         self,
#         sft_hidden_states: torch.Tensor,
...
#         """
#         为单个层计算PCA正则化损失。
#         所有输入都应为GPU上的PyTorch张量。
#         """
#         batch_size, seq_len, hidden_dim = sft_hidden_states.shape
        
#         # 1. 展平并过滤掉padding token对应的hidden_states
#         # attention_mask中为1的位置是有效token
#         valid_indices = attention_mask.view(-1).bool()
#         flat_sft_states = sft_hidden_states.view(-1, hidden_dim)[valid_indices]
        
#         if flat_sft_states.numel() == 0:
#             return torch.tensor(0.0, device=self.model.device)

#         # 应用剪枝掩码
#         masked_pca_mean = pca_mean * pruning_mask
        
#         # 中心化：用屏蔽后的均值对SFT状态进行中心化
#         centered_states = flat_sft_states - masked_pca_mean
        
#         # 投影：将中心化后的状态投影到原始模型的PCA主成分上
#         projected_states = torch.matmul(centered_states, pca_components.T)
        
#         # 计算损失：SFT模型在原始模型的主空间中的投影尽可能小
#         pca_loss = torch.mean(torch.sum(projected_states**2, dim=-1))
...
#         重写此方法来加入PCA正则化损失。
#         """
#         # --- 步骤1：正常的前向传播，并获取hidden_states ---
#         # 确保模型调用时返回hidden_states
#         outputs = model(**inputs, output_hidden_states=True)
#         # print(model)
#         # --- 步骤2：计算原始的任务损失 (例如：交叉熵损失) ---
#         if hasattr(outputs, "loss") and outputs.loss is not None:
#             original_loss = outputs.loss
            
#         # --- 步骤3：计算PCA正则化损失 ---
#         total_pca_loss = torch.tensor(0.0, device=model.device)
        
#         # 从模型输出中获取SFT模型的隐藏状态列表和注意力掩码
#         sft_all_hidden_states = outputs.hidden_states
#         attention_mask = inputs.get("attention_mask")

#         # 假设您的模型有一个方法来获取所有层的剪枝掩码
#         # 这是需要您根据实际情况调整的地方！
#         # 例如: pruning_masks -> {5: mask_tensor_5, 15: mask_tensor_15, ...}
#         pruning_mask = model.module.base_model.model.model.mask # 这是一个示意函数
...
#         if update_step > 0.02 * tap_stop_at_steps:
#             self.pca_lambda = 0.001
#         # print("Pruning_mask",pruning_mask)
#         for layer_idx in self.target_layers:
#             # 验证所需数据是否存在

#             # 获取当前层所需的所有张量
#             sft_hidden_states = sft_all_hidden_states[layer_idx]
#             pca_mean = self.pca_params[layer_idx]["mean"]
#             pca_components = self.pca_params[layer_idx]["components"]

#             # 计算单层的PCA损失
#             layer_pca_loss = self._compute_single_layer_pca_loss(
#                 sft_hidden_states,
#                 attention_mask,
#                 pruning_mask,
#                 pca_mean,
#                 pca_components
#             )
#             total_pca_loss += layer_pca_loss

#         # --- 步骤4：组合损失 ---
#         # 所有操作都是在GPU上用PyTorch张量完成的，计算图是连续的
#         final_loss = original_loss + self.pca_lambda * total_pca_loss
        
#         # 记录损失值，方便在wandb或tensorboard中观察
#         if self.state.global_step % self.args.logging_steps == 0:
#             self.log({
#                 "original_loss": original_loss.item(),
#                 "pca_loss": total_pca_loss.item(),
