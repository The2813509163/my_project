...
class AdvancedParameterDistributionDPOLoss(nn.Module):
    ...
    def kl_divergence_reward(self, current_params):
        ...
        for name, current_param in current_params.items():
            if name in self.initial_params:
                ...
                kl_div = F.kl_div(
                    torch.log(current_hist + self.epsilon),
                    initial_hist,
                    reduction='sum'
                ).item()
                
                # 方法2: 基于参数值直接转换的概率分布
                initial_prob = self._tensor_to_distribution(self.initial_params[name])
                ...
                # 组合两种方法
                combined_kl = 0.7 * kl_div + 0.3 * kl_div_direct
                
                weight = self.layer_weights.get(name, 1.0)
                total_kl += combined_kl * weight
                total_weight += weight
        
        avg_kl = total_kl / total_weight if total_weight > 0 else 0
    ...
    def forward(self, model, task_loss=None, return_details=False):
        ...
        dpo_loss = -F.logsigmoid(torch.tensor(self.beta * reward_diff, dtype=torch.float32))
        ...
        if return_details:
            ...
            if task_loss is not None:
                total_loss = task_loss + dpo_loss
                details['task_loss'] = task_loss.item()
                details['total_loss'] = total_loss.item()
                return total_loss, details
            else:
                return dpo_loss, details
