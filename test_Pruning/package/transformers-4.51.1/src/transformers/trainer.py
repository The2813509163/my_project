...
class ZipfTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.teacher_model = AutoModelForCausalLM.from_pretrained("/data/kris/shared_data/models/Llama-3.2-3B",
    ...
    def compute_loss(self, model, inputs, return_outputs=False,num_items_in_batch=None):
        ...
        # print("LLLLLLLLLLLLL:",target_sequence_length)
        # 4. 生成一个批次的dummy数据
        ...
        print(f"生成执行耗时: {elapsed_time:.4f} 秒")
        # --- 接下来的逻辑和之前一样 ---
        # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True) as prof:
        ...
        # print(model.base_model.model.model.layers[0].self_attn.q_proj.weight.dtype)
        ...
        # loss = model.base_model.model.loss_function(logits=logits_finetune, labels=finetune_labels, vocab_size=model.base_model.model.config.vocab_size)
        ...
        # # 计算并打印耗时
        elapsed_time = end_time - start_time
        print(f"蒸馏执行耗时: {elapsed_time:.4f} 秒")
...
class AdvancedDummyTrainer(Trainer):
    """
    一个自定义的Trainer，用于实现知识蒸馏。
    它同时计算标准的微调损失和与教师模型的KL散度损失。
    """
    def __init__(
        ...
        **kwargs,
        ...
        self.alpha = alpha
        self.temperature = temperature
        self.sft_batch_size = kwargs.get('sft_batch_size', 6)
        # 确保教师模型在正确的设备上，并处于评估模式
        self.teacher_model.eval()
    ...
    def compute_loss(self, model, inputs, return_outputs=False,num_items_in_batch=None):
        ...
        """
        # === 第一部分：计算标准微调损失 (L_finetune) ===
        sft_size = self.sft_batch_size
        model_inputs = {
            'input_ids': inputs['input_ids'],
            'attention_mask': inputs['attention_mask']
        }
        # finetune_inputs = model_inputs[:sft_size]
        dummy_inputs = model_inputs[sft_size:]
        finetune_labels = inputs['labels'][:sft_size]
        # === 第二部分：实时生成dummy数据并计算蒸馏损失 (L_distill) ===
        # print("BBBBBBB",model.base_model.model.config.vocab_size)
        ...
        pruning_loss,outputs = super().compute_loss(model, model_inputs, return_outputs=True,num_items_in_batch=None)
      

        logits_finetune = outputs.logits[:sft_size]
        logits_student = outputs.logits[sft_size:]
