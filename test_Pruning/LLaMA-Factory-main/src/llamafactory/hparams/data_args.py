...
class DataArguments:
    ...
    # my_modify add option fixed
    mix_strategy: Literal["concat", "interleave_under", "interleave_over","fixed"] = field(
        default="concat",
        metadata={"help": "Strategy to use in dataset mixing (concat/interleave) (undersampling/oversampling).Use 'fixed' for custom SFT/PT ratio mixing."},
    )
    # my_modify
    sft_batch_size: Optional[int] = field(
        default=4,
        metadata={"help": "The ratio of SFT samples in a fixed-mixing batch (e.g., 3 for a 4:1 SFT/PT ratio)."},
    )
